...
namespace utility
{
    public class myinclude
    {
        ...
        public byte[] file_get_contents_post(string url, string postData)
        {
            ...
            httpWReq.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.93 Safari/537.36";
            httpWReq.Proxy = null;
                ...
                using (TcpClient client = new TcpClient(uri.Host, uri.Port))
                {
                    using (SslStream sslStream = new SslStream(client.GetStream(), false, ValidateServerCertificate, null))
                    {
                        ...
                        if (certificate != null)
                        {
                            ...
                            string expirationDate = cert2.GetExpirationDateString();
                            return DateTime.Parse(expirationDate).ToString("yyyy-MM-dd HH:mm:ss");
                        }
                    }
                }
            }
        ...
        public string get_host_expire_from_url(string url)
        {
            ...
            try
            {
                ...
                while (!streamReaderReceive.EndOfStream)
                {
                    ...
                    {
                        return trim(get_between(data, "expires on ", "("));
                    }
